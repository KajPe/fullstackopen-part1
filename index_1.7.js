...
class App extends React.Component {
  ...
  render() {
    const lukuja = this.state.hyva + this.state.neutraali + this.state.huono
    const ka = ((this.state.hyva - this.state.huono) / lukuja).toFixed(1)
    const positiv = ((this.state.hyva / lukuja ) * 100).toFixed(1).concat("%")
    ...
    return (
      <div>
        ...
        <div>
          ...
          <Statistic text="Positiivisia" value={positiv} />
        </div>
      </div>
    )
  }
